...
exports.insert_games = void 0;
const seed_1 = require("./seed");
async function insert_games() {
    console.log('Find users');
    const emma = await seed_1.prisma.user.findUnique({ where: { id: 1 } });
    const lucie = await seed_1.prisma.user.findUnique({ where: { id: 2 } });
    const fabien = await seed_1.prisma.user.findUnique({ where: { id: 3 } });
    const math = await seed_1.prisma.user.findUnique({ where: { id: 4 } });
    console.log('Creating games');
    const game1 = await seed_1.prisma.game.create({
        data: {
    ...
    const game2 = await seed_1.prisma.game.create({
        data: {
            playerOne: { connect: { id: emma.id } },
    ...
    const game3 = await seed_1.prisma.game.create({
        data: {
    ...
    const game4 = await seed_1.prisma.game.create({
        data: {
            playerOne: { connect: { id: lucie.id } },
    ...
    const game5 = await seed_1.prisma.game.create({
        data: {
            playerOne: { connect: { id: lucie.id } },
            playerTwo: { connect: { id: math.id } },
            winner: { connect: { id: math.id } },
...
exports.insert_games = insert_games;
